...
public class Prac3 {
	static int avg(int[] arr) { //산술평균 메소드
		...
		return (int) result; // int형으로 타입 캐스트
	}
	
	// ? Math.round를 하고 왜 Math.ceil를 하는지? 
	// ? result를 왜 double을 하고, 마지막 return에서는 int를 하는 지? 
	
	static int middle(int[] arr) { // 중앙값을 구하는 메소드
		
		//arr를 오름차순으로 시작할 것 이다. 즉, arr.length는 총 길이가 나오니까 
		//거기에서 2를 나눈 '곳'의 값이 중앙값이다. 
